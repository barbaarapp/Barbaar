...
export default function App() {
  ...
  useEffect(() => {
    // Safety: ensure loading state doesn't stay true forever
    const timer = setTimeout(() => {
      setLoading(false);
    }, 5000); // 5 second max for splash

    // Check Firebase Connection with a small delay for stability
    setTimeout(() => {
      import('./firebase').then(({ checkConnection }) => {
        checkConnection().then(connected => {
          if (!connected) {
            console.error("Firebase connection check failed. Your application may be offline or the database is currently unreachable.");
          } else {
  ...
  const fetchUserData = async (userId: string) => {
    ...
    try {
      ...
      try {
        ...
        // Attempt to get as a final fallback
        docSnap = await getDoc(docRef); 
      }
      ...
      if (!docSnap.exists()) {
        ...
        if (isNewDay) {
          ...
          const welcomeReward = updateGamification(mapFromFirestore({ ...data, id: userId }) as UserProfile, 'DAILY_WELCOME');
          ...
          updateUserState({ ...data, ...updates, id: userId });
        } else {
      ...
      // Fetch Challenges
      const challengesQuery = query(collection(db, 'challenges'), where('published', '==', true));
      const challengesUnsub = onSnapshot(challengesQuery, (snapshot) => {
    ...
    return (
      <div className="min-h-screen bg-bg flex flex-col items-center justify-center p-6 text-center">
        <motion.div
           ...
           className="space-y-6"
        >
          <div className="w-20 h-20 bg-card rounded-[2rem] shadow-xl flex items-center justify-center mx-auto">
            <Loader2 className="animate-spin text-brand" size={40} />
          </div>
          <div className="space-y-2">
            <p className="text-[10px] font-black text-text/30 uppercase tracking-[0.4em]">Barbaar</p>
            <p className="text-xs font-bold text-text/60 italic lowercase">nurturing your path to wellness...</p>
          </div>
        </motion.div>
      </div>
    );
