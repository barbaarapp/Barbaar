...
export default function SettingsScreen({
  ...
  React.useEffect(() => {
    ...
    const initializeGsi = () => {
      ...
      try {
        ...
        // Trigger Google One Tap for instant in-page pop-up accounts list
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (err.code === "auth/popup-blocked") {
        setAuthError("Google Sign-In popup was blocked by your browser. Please click the button again or use our Email/Demo accounts option below.");
      } else {
  ...
  async function handlePresetLogin(email: string) {
    ...
    try {
      ...
      // Auto-register preset on first use
      try {
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <Card style={{ padding: 16 }}>
            {currentUser ? (
              ...
              // Logged Out state UI (The premium login cards)
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <p style={{ fontSize: "12.5px", color: colors.inkSoft, lineHeight: 1.45 }}>
                  {t("Sign in with your Gmail account to instantly secure your session calendar, join video consultations, and access direct therapist chat channels.", lang)}
                </p>
                ...
                {/* Modern Official In-Page Google Sign-In Button */}
                <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "center", width: "100%", marginTop: 4 }}>
                  <div 
                  ...
                  <div style={{ fontSize: "11px", color: colors.inkSoft, textAlign: "center", lineHeight: 1.35, marginTop: 2 }}>
                    {t("A secure, one-click login pop-up or dropdown will appear in-page instantly.", lang)}
                  </div>
                </div>
                ...
                <button
                  onClick={() => {
                    setAuthMode(authMode === "none" ? "email" : "none");
                    setAuthError(null);
                  }}
                  style={{
                    background: "none",
                ...
                >
                  {authMode === "email" ? t("Use Google Sign-In instead", lang) : t("Use Email or Demo Accounts", lang)}
                </button>

                {/* Collapsible Email and Demo Sign-in container */}
                {authMode === "email" && (
                  <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 10, background: colors.indigoSoft, padding: 14, borderRadius: 14 }}>
                    <TextField
                      label={t("Email address", lang)}
                      value={emailInput}
                    ...
                    <Button
                      ...
                      onClick={handleEmailSignInOrUp}
                      style={{ height: 38, fontSize: "12.5px" }}
                    >
                      {authLoading ? "Authenticating..." : t("Sign In / Create Account", lang)}
                    </Button>

                    <div style={{ height: 1, background: `${colors.line}50`, margin: "6px 0" }} />

                    {/* Pre-configured clinical and administrative testing presets */}
                    <button
                    ...
                    >
                      <span>⚡ {showDemoOptions ? "Hide Demo Accounts" : "Show Quick Demo Accounts"}</span>
                    </button>

                    {showDemoOptions && (
                      <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 4 }}>
                        <div style={{ fontSize: "10.5px", color: colors.inkSoft, textAlign: "center" }}>
                          Select a role preset below to instantly simulate the account authorization rules:
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                          <button
                            onClick={() => handlePresetLogin("barbaaryp@gmail.com")}
                            style={{
                              background: colors.amberSoft,
                              color: colors.amber,
                          ...
                          <button
                            ...
                            style={{
                              ...
                              color: colors.acacia,
                              border: `1px solid ${colors.acacia}30`,
                              borderRadius: 10,
                              padding: "8px 4px",
                              fontSize: "11px",
                              fontWeight: 700,
                        ...
                        <div style={{ display: "flex", justifyContent: "center" }}>
                          <button
                            onClick={() => handlePresetLogin("test-client@barbaar.com")}
                            style={{
                              background: colors.indigoSoft,
                              color: colors.indigo,
