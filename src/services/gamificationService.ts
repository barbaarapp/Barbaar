...
export const calculateGrowthScore = (profile: UserProfile): number => {
  ...
  const streakScore = Math.min(((profile.streak || 0) + (profile.journalStreak || 0)) / 45, 1) * 100;
  ...
  const pointsScore = Math.min((profile.sabrPoints || 0) / 10000, 1) * 100;
  
  // Activity contribution - Completed resources and Word count progress
  const resourceScore = Math.min((profile.completedResources?.length || 0) / 15, 1) * 100;
  const wordScore = Math.min((profile.totalWordCount || 0) / 5000, 1) * 100;
